...
for start in range(0, max_page, 10):
    ...
    #grabbing posting date
    for div2 in soup.find_all(name='div', attrs={'class':'heading6 tapItem-gutter result-footer'}):
        for span1 in div2.find_all(name='span', attrs={'class':'date'}):
            post_dates11.append(span1.text.strip())
...
# create dataframe
job_list_analyst_df = pd.DataFrame(list(zip(jobs11, companies11, locations11, summaries11_noreturn, post_dates11)),
               columns = ['job_title', 'company_name', 'location', 'summary', 'posted'])
...
for start in range(0, max_page, 10):
    ...
    for div2 in soup.find_all(name='div', attrs={'class':'heading6 tapItem-gutter result-footer'}):
        for span1 in div2.find_all(name='span', attrs={'class':'date'}):
            post_dates12.append(span1.text.strip())
...
# create dataframe
job_list_sci_df = pd.DataFrame(list(zip(jobs12, companies12, locations12, summaries12_noreturn, post_dates12)),
               columns = ['job_title', 'company_name', 'location', 'summary', 'posted'])
...
for start in range(0, max_page, 10):
    ...
    for div2 in soup.find_all(name='div', attrs={'class':'heading6 tapItem-gutter result-footer'}):
        for span1 in div2.find_all(name='span', attrs={'class':'date'}):
            post_dates14.append(span1.text.strip())
...
# create dataframe
job_list_eng_df = pd.DataFrame(list(zip(jobs14, companies14, locations14, summaries14_noreturn, post_dates14)),
               columns = ['job_title', 'company_name', 'location', 'summary', 'posted'])
...
job_list_all_df.to_csv('dates_job_list_all.csv', index=False)
